...
class VetorNaoOrdenado:
  ...
  #busca por hash com trtamento de colisão.
  def busca_hash(self, valor):
    pos = valor % self.capacidade # Posição inicial do vetor.
    if self.valores[pos] == valor: # Caso o valor já exista na lista, informa.
      return pos # Retorna posição.
    else: # Caso o valor não exista na lista, procura na próxima posição.
      for i in range(self.capacidade): # Percorre todo o vetor.
        if self.valores[pos] == valor: # Caso encontre o valor, informa.
          return pos # Retorna posição.
        pos = (pos + 1) % self.capacidade # Caso não encontre o valor, procura na próxima posição.
      return -1 # Caso não encontre o valor, informa.
...
lista = VetorNaoOrdenado(100)
lista.gera_dados(20)
# lista.imprimir()

print('\nBusca binária:', lista.busca_hash(999))